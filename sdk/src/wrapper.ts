...
const FRAMEWORK_BUDGETS: Record<string, number> = {
  openai: 0.35,
  langgraph: 0.30,
  crewai: 0.25,
  generic: 0.20,
...
function createWrappedAgent(
  ...
  const wrappedExecute = async (task: string): Promise<TaskResult> => {
...
export function wrapGeneric(
  ...
  return createWrappedAgent(
    { ...agent, framework: agent.framework ?? 'generic' },
    fleet,
    role,
